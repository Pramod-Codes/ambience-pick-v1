...
export function TableNode({
  ...
  if (table.shape === "seat") {
    return (
      ...
      >
        ...
        <span className={cn("absolute -right-1 top-1/2 h-4 w-1.5 -translate-y-1/2 rounded-full", styles.chair)} />
        {table.id}
      </button>
    );
  }
  ...
  if (table.shape === "rect") {
    const perSide = Math.max(1, Math.round(table.capacity / 2));
    return (
      <button
        onClick={onClick}
        disabled={disabled}
  ...
  return (
    ...
    >
      {Array.from({ length: table.capacity }).map((_, i) => {
        ...
        return <Chair key={i} className={cn("h-2.5 w-2.5", styles.chair)} style={{ left: `${50 + 48 * Math.cos(angle)}%`, top: `${50 + 48 * Math.sin(angle)}%`, transform: "translate(-50%, -50%)" }} />;
      })}
      <div
        className={cn(
          "flex h-full w-full items-center justify-center rounded-xl text-sm font-bold text-white shadow-soft transition-transform active:scale-95",
